...
int is_heap_empty_no_swap_version(const binheap_type *H)
{
    return H->num_of_elem == 0;
}

const void *min_value_no_swap_version(const binheap_type *H)
{
    if (is_heap_empty_no_swap_version(H)){
        return NULL;
    }
...
void swap_keys_no_swap_version(binheap_type *H, unsigned int n_a, unsigned int n_b)
{
...
void heapify_no_swap_version(binheap_type * H, unsigned int node)
{
    ...
    do {
        ...
        // swap the keys
        if(dst_node != node){
            swap_keys_no_swap_version(H, dst_node, node);
        }
    } while(dst_node != node);
}

const void *extract_min_no_swap_version(binheap_type *H)
{
    if (is_heap_empty_no_swap_version(H)){
        return NULL;
    }
    ...
    swap_keys_no_swap_version(H, 0, H->num_of_elem-1);
    ...
    heapify_no_swap_version(H, 0);

    return ADDR(H, H->num_of_elem+1);
}

const void * find_the_max_no_swap_version(void *A, 
...
binheap_type *build_heap_no_swap_version(void *A, 
                         const unsigned int num_of_elem,
                         ...
                         total_order_type leq)
{
	...
	const void * value = find_the_max_no_swap_version(A, num_of_elem, key_size, leq);
...
void delete_heap_no_swap_version(binheap_type *H)
{
...
const void *decrease_key_no_swap_version(binheap_type *H, void *node, const void *value)
{
    ...
    while((node_idx != 0) && (!H->leq(parent, node))){
    	...
    	swap_keys_no_swap_version(H, parent_idx, node_idx);
...
const void *insert_value_no_swap_version(binheap_type *H, const void *value)
{
    ...
    return decrease_key_no_swap_version(H, new_node_addr, value);
}

void print_heap_no_swap_version(const binheap_type *H, 
