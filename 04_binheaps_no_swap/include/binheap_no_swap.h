#ifndef __BIN_HEAP__NO_SWAP
#define __BIN_HEAP__NO_SWAP
...
typedef struct {
    ...
    unsigned int * key_pos; // this is the array used to store the position of the key of a node
    unsigned int * rev_pos; // this is the array used to store the node corresponding to a given position
    unsigned int removed_elem; // this is the number of nodes that has been removed from the heap
}binheap_type;
 ...
 **********************************************************************/
void print_heap_no_swap_version(const binheap_type *H, void (*key_printer)(const void *value));
